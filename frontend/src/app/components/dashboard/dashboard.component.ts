...
export class DashboardComponent implements OnInit {
  ...
  displayedSubjects = computed(() => {
    ...
    return this.showAllSubjects() ? all : all.slice(0, 8);
  });

  // Tab navigation
  activeTab = signal<'dashboard' | 'timer' | 'chrono' | 'ai' | 'subjects'>('dashboard');
  showMobileMoreMenu = signal<boolean>(false);

  toggleMobileMoreMenu() {
    this.showMobileMoreMenu.set(!this.showMobileMoreMenu());
  }

  selectMobileTab(tab: 'chrono' | 'ai') {
  ...
  aiLastSource = signal<'smart' | 'gemini' | 'fallback' | null>(null);
  ...
  // Material Icons for subjects — 60 icons across all domains
  icons = [
    // Academic
    'menu_book', 'calculate', 'science', 'computer', 'public',
    'book', 'psychology', 'language', 'history_edu', 'school',
    'functions', 'biotech', 'architecture', 'auto_stories', 'category',
    // Arts & Creativity
    'palette', 'music_note', 'edit', 'brush', 'photo_camera',
    'movie', 'piano', 'theater_comedy', 'color_lens', 'draw',
    // Science & Tech
    'straighten', 'computer', 'memory', 'code', 'terminal',
    'analytics', 'data_object', 'hub', 'rocket_launch', 'satellite',
    // Sports & Health
    'sports_soccer', 'fitness_center', 'sports_basketball', 'self_improvement', 'directions_run',
    'sports_tennis', 'pool', 'sports_esports', 'hiking', 'sports_martial_arts',
    // Life & Misc
    'lightbulb', 'star', 'favorite', 'emoji_events', 'flag',
    'travel_explore', 'eco', 'agriculture', 'restaurant', 'home',
    // Tools
    'build', 'engineering', 'design_services', 'tune', 'settings_suggest',
    'extension', 'account_tree', 'workspaces', 'grid_view', 'layers'
  ];

  // 40 curated colors — vibrant, harmonious, accessible
  colors = [
    // Purples & Indigos
    '#7c3aed', '#8b5cf6', '#6366f1', '#4f46e5', '#a855f7',
    // Blues
    '#2563eb', '#3b82f6', '#0284c7', '#0891b2', '#06b6d4',
    // Greens
    '#059669', '#10b981', '#16a34a', '#65a30d', '#84cc16',
    // Reds & Pinks
    '#dc2626', '#ef4444', '#e11d48', '#db2777', '#ec4899',
    // Oranges & Yellows
    '#ea580c', '#f97316', '#f59e0b', '#ca8a04', '#ffd700',
    // Teals & Cyans
    '#0d9488', '#14b8a6', '#22d3ee', '#38bdf8', '#7dd3fc',
    // Rose & Fuchsia
    '#c026d3', '#d946ef', '#a21caf', '#be185d', '#9d174d',
    // Slate & Cool
    '#475569', '#64748b', '#6b7280', '#4b5563', '#374151'
  ];

  // Show-more toggles for modal
  showMoreIcons = signal(false);
  showMoreColors = signal(false);

  get visibleIcons() { return this.showMoreIcons() ? this.icons : this.icons.slice(0, 20); }
  get visibleColors() { return this.showMoreColors() ? this.colors : this.colors.slice(0, 16); }
  ...
  get aiSuggestions() {
    const isFr = this.languageService.currentLanguage() === 'fr';
    return [
      { icon: 'auto_graph', text: isFr ? 'Analyser ma journée' : 'Analyze my day' },
      { icon: 'style', text: isFr ? 'Que réviser en priorité ?' : 'What to revise first?' },
      { icon: 'event', text: isFr ? 'Un plan pour demain' : 'A plan for tomorrow' },
      { icon: 'local_fire_department', text: isFr ? 'Comment va ma régularité ?' : 'How is my consistency?' }
    ];
  }

  requestAIAdvice() {
    const isFr = this.languageService.currentLanguage() === 'fr';
    this.askAI(isFr ? 'Analyser ma journée' : 'Analyze my day');
  }

  askAI(question: string) {
    const trimmed = question.trim();
    if (!trimmed || this.aiLoading()) return;

    this.aiInputText.set('');
    
    // Add user message
    this.aiMessages.update(msgs => [...msgs, { role: 'user', content: trimmed }]);
    this.scrollAiChatToEnd();

    // Set loading state
    this.aiLoading.set(true);

    // Push placeholder for thinking indicator
    this.aiMessages.update(msgs => [...msgs, { role: 'assistant', content: '', isThinking: true }]);
    this.scrollAiChatToEnd();

    const context = this.buildAIContext();
    
    // Map current messages to history payload (excluding the thinking card and the current prompt)
    const history = this.aiMessages()
      .filter(m => !m.isThinking)
      .slice(0, -1) // exclude the latest user question from history
      .map(m => ({ role: m.role, content: m.content }));

    this.aiAssistantService.getStudyAdvice({
      context,
      question: trimmed,
      history
    }).subscribe({
      next: (res) => {
        ...
        this.aiMessages.update(msgs => msgs.filter(m => !m.isThinking));
        
        if (res.success && res.advice) {
          this.aiLastSource.set(res.source);
          this.streamAIResponse(res.advice);
        } else {
          this.aiLastSource.set('smart');
          const fallback = this.generateSmartAdvice(context);
          this.streamAIResponse(fallback);
        }
      },
      error: (err) => {
  ...
  private streamAIResponse(fullText: string) {
    ...
    this.activeStreamTimer = setInterval(() => {
      ...
      this.aiMessages.update(msgs => {
        ...
        if (last.role === 'assistant') {
          ...
          return [
            ...msgs.slice(0, -1),
            { ...last, content: last.content + separator + words[currentIndex] }
          ];
        }
        return msgs;
      });
  ...
  scrollAiChatToEnd() {
    setTimeout(() => {
      const chatBody = document.querySelector('.db-ai-messages-tall');
      if (chatBody) {
        chatBody.scrollTo({
